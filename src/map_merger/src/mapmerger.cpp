...
MapMerger::MapMerger()
{
    ROS_INFO("[MapMerger] Constructor");
    pos_seq_my = 0;
   ...
   // position_other_robots_topic = "/robot_1/position_other_robots";
    ...
    ROS_INFO("[MapMerger] Local map topic:%s",local_map_topic.c_str());
    ROS_INFO("[MapMerger] Local map meta data:%s",local_map_metadata_topic.c_str());
    ROS_INFO("[MapMerger] Width:%i",map_width);
    ROS_INFO("[MapMerger] Height:%i",map_height);
    ROS_INFO("[MapMerger] Splitted:%i",splitted);
    ROS_INFO("[MapMerger] Has local map:%i",has_local_map);
    ROS_INFO("[MapMerger] Split size:%i",size);
    ROS_INFO("[MapMerger] Robot name:%s",robot_name.c_str());
    ROS_INFO("[MapMerger] Prefix:%s",robot_prefix.c_str());
    ROS_INFO("[MapMerger] Local Map Frame:%s",local_map_frame_id.c_str());
    robot_hostname = robot_name;
...
{
    //ros::NodeHandle nodeHandle;
    ROS_INFO("[MapMerger] Wait for map_meta_data, on topic:[%s]",local_map_metadata_topic.c_str());
    ...
    ROS_INFO("[MapMerger] Got map_meta_data");
    if(!splitted)
    {
        size = map_height;
    }
...
{
    ROS_INFO("[MapMerger] Wait to get a map from other robot");
    ...
    {
        ROS_INFO("[MapMerger] %lu robots are already in the network, adding them,my name:%s",getNeighbors.response.neigbors.size(),
                 robot_name.c_str());
                    ...
                    if(has_local_map == false)
                        {
                            ...
                            ROS_INFO("[MapMerger] now using %s as name",robot_name.c_str());
                            return;
                        }
                }
            }
        ...
        {
            ROS_INFO("[MapMerger] No robots are were in the network before me");
        }
    }
    while(ros::ok())
    {
        ROS_INFO("[MapMerger] No robot information");
        ros::spinOnce();
        ros::Duration(0.3).sleep();
        if(robot_name != "BASE")
            break;
...
{
    ROS_INFO("[MapMerger] Using %s as pseudo local map",msg.get()->data.c_str());
    this->robot_name = msg.get()->data;
}
void MapMerger::callback_new_robot(const std_msgs::StringConstPtr &msg)
{
    ROS_INFO("[MapMerger] callback_new_robot");
    ...
    ROS_INFO("[MapMerger] New robot:%s",newRobotName.c_str());
    updateMan->addNewUpdateList();
    makeEmptyMapData(newRobotName,map_height,map_width,local_map->info.resolution);
    ROS_INFO("[MapMerger] asking %s for map in new Robot",newRobotName.c_str());
    ...
    ROS_INFO("[MapMerger] Send Control Message in new Robot");
...
{
    ROS_INFO("[MapMerger] callback_control");
    try {
    ...
    if(tmp.update_numbers.size() == 0)
    {
        ...
        ROS_INFO("[MapMerger] Sended map after beeing asked to %s,containd %lu updates,update_seq:%i",tmp.src_robot.c_str(),containedUpdates->size(),update_seq);
...
{
    try {
    ROS_INFO("[MapMerger] callback_global_pub");
   // sendMetaData(5);
    if(robots->size() == 0)
    {
        ROS_INFO("[MapMerger] No other robots, publishing local map");
    ...
    if(map_data->size() > 0 && transforms->size() > 0)
    {
        ...
        for(int i = 0; i < transforms->size(); i++)
        {
            ...
            ROS_INFO("[MapMerger] Merging [%s] map",name.c_str());
    ...
    ROS_ERROR("CRASH while callback_global_pub");
}
}
...
{
    try {
    ROS_INFO("[MapMerger] Sending Position, x:%f|y:%f",cur_position->pose.position.x,cur_position->pose.position.y);
    ...
    ROS_INFO("[MapMerger] END callback_send_position");
}

void MapMerger::callback_recompute_transform(const ros::TimerEvent &e)
{
    ROS_INFO("[MapMerger] callback_recompute_transform");
    if(transforms->size() == 0)
    {
        ROS_DEBUG("No transforms to recompute");
        return;
    }
    ROS_INFO("[MapMerger] Recompute transforms");
...
{
    ROS_INFO("[MapMerger] callback_send_map");
...
*/
    ...
    if(local_map->data.size() != local_map_old->data.size())
    {
        ...
        ROS_INFO("[MapMerger] Deleting global_map");
        delete global_map;
        global_map = new nav_msgs::OccupancyGrid();
        global_map->data.resize(local_map->data.size());
        ROS_INFO("[MapMerger] Copying local map data into global map");
        std::copy(local_map->data.begin(),local_map->data.end(),global_map->data.begin());
        ROS_INFO("[MapMerger] Clearing transforms");
        transforms->clear();
        robots_in_transform->clear();
        nav_msgs::OccupancyGrid * whole_map;
        ROS_INFO("[MapMerger] Start recalcing the transformations");
        for(int i = 0; i < robots->size(); i++)
        {
            ROS_INFO("[MapMerger] Merge %i in map_data",i);
            computeTransform(i);
...
{
    ROS_INFO("[MapMerger] callback_got_position_network");
    //ros::nodeHandle nodeHandle ("~");
...
{
    ROS_INFO("[MapMerger] callback_map_other");
    nav_msgs::OccupancyGrid tmp =  msg.get()->map;
    nav_msgs::OccupancyGrid * toInsert = &tmp;
    int index_robots = -1;
    ROS_INFO("[MapMerger] Got map from %s",msg.get()->src_robot.c_str());
    ...
    if(has_local_map == false)
    {
        if(src_robot == robot_name)
        {
            ...
            {
                ROS_INFO("[MapMerger] Local map is null");
                ...
                ROS_INFO("[MapMerger] Resize done");
                //here recalc Indexes!!!
                ...
                ROS_INFO("[MapMerger] Local map is %ix%i",toInsert->info.height,toInsert->info.width);
            }
            else
            {
    ...
    else
    {
        ...
        ROS_INFO("[MapMerger] GOT NOT LOCAL MAP,Process it, data size:%lu",toInsert->data.size());
...
{
    ROS_INFO("[MapMerger] Start callback_map");
    //get the message
    nav_msgs::OccupancyGrid tmp = *msg.get();
    nav_msgs::OccupancyGrid * toInsert = &tmp;
    //insert it into the list
    int index_robots = -1;
    //ROS_INFO("[MapMerger] Got map, frame_id:%s|size of data:%i",toInsert->header.frame_id.c_str(),toInsert->data.size());
...
{
    ROS_INFO("[MapMerger] Process Local Map");
    if(!has_local_map)
        return;
    ...
    if(global_map == NULL)
    {
        ...
        ROS_INFO("[MapMerger] %p|%p",&local_map->data,&local_map_old->data);
        global_map = new nav_msgs::OccupancyGrid();
        global_map->data = local_map->data;
        global_map->header = local_map->header;
        global_map->info = local_map->info;
...
void MapMerger::processMap(nav_msgs::OccupancyGrid *map,int index_in_mapdata)
{
    ...
    ROS_INFO("[MapMerger] Process Map Splitted");
...
{
    ROS_INFO("[MapMerger] processPosition");
    //ros::nodeHandle nodeHandle ("~");
...
{
    //ros::NodeHandle nodeHandle;
    ROS_INFO("[MapMerger] Started MapMerger");
    ROS_INFO("[MapMerger] Subscribe map topic [%s]",local_map_topic.c_str());
    ...
    ROS_INFO("[MapMerger] Init Sub_other_maps");
...
//#endif
    ...
    {
        ROS_INFO("[MapMerger] Creating position stuff");
    ...
    ROS_INFO("[MapMerger] Init Subscriber");
   ...
   // ros::Publisher pub_position = nodeHandle->advertise<geometry_msgs::Pose>("position_over_network",3);
    ...
    {
        ROS_INFO("[MapMerger] Create timer to recompute the transformations all %i seconds",seconds_recompute_transform);
    ...
    while(local_map->data.size() == 0 && has_local_map)
    {
        ...
        ros::spinOnce();
        ROS_INFO("[MapMerger] Local_Map size = 0");
    }
    ros::Duration(0.1).sleep();
    ros::AsyncSpinner spinner(10);
    spinner.start();
...
{
    ROS_INFO("[MapMerger] Start merging map: [%s]",mapToMerge->header.frame_id.c_str());
    if(global_map == NULL)
    {
        ROS_INFO("[MapMerger] Stop merging maps, globl map is null");
        return;
    }
    int collums,rows;
    if(max_x == -1 || max_y == -1)
    ...
    if(mapToMerge->header.frame_id != local_map_frame_id)
     ...
     ROS_INFO("[MapMerger] Merged maps succesfully");
}

cv::Mat MapMerger::mapToMat(const nav_msgs::OccupancyGrid *map)
{
    ROS_INFO("[MapMerger] mapToMat");
...
{
    ROS_INFO("[MapMerger] transformImage");
...
{
    ROS_INFO("[MapMerger] Start updateMap");
...
{
    ROS_INFO("[MapMerger] Start in getMapPart, creating map part, min_x:%i|max_x:%i|min_y:%i|max_y:%i|Index:%i",start_x,start_x+height,start_y,start_y+width,map_index);
...
{
    ROS_INFO("[MapMerger] updateMapArea");
...
{
    ROS_INFO("[MapMerger] recomputeTransform");
    ...
    if(worked)
    {
        ROS_INFO("[MapMerger] Storing new Transform for %s in array",map->header.frame_id.c_str());
       ...
       ROS_INFO("[MapMerger] NO Transform for %s",map->header.frame_id.c_str()); 
    }
    return worked;
}
...
{
    ROS_INFO("[MapMerger] Start computeTransform for mapDataIndex:%i",mapDataIndex);
    int sum_elements = std::accumulate(local_map->data.begin(),local_map->data.end(),0);
    if(local_map->data.size()== 0
            || sum_elements == (-1) * local_map->data.size())
    {
...
{
    ROS_INFO("[MapMerger] Creating emptyMapData for [%s]",robot_name.c_str());
...
void MapMerger::callback_robot_status(const nav_msgs::MapMetaData::ConstPtr &msg)
{
    for(int i = 0; i < robots->size(); i++)
    {
        ...
        {
            ROS_INFO("[MapMerger] Robot known");
        }
        else
        {
            ROS_INFO("[MapMerger] New Robot:%s",robot_name.c_str());
...
{
    ROS_INFO("[MapMerger] sendMapOverNetwork");
     //ros::nodeHandle nodeHandle ("~");
        ...
        for(int row = start_row; row < end_row;row+=size)
        {
            for(int collum = start_collum; collum < end_collum;collum+=size)
            {
                ...
                */
                ...
                ROS_INFO("[MapMerger] Send Map to %s(if nothing -> boradcast) ,frame id:%s| topic:[%s]|data<_size:%lu",
                ...
                if(client.call(exchange))
                {
                    if(exchange.response.status)
                        if(destination == "")
                            ROS_INFO("[MapMerger] Sended Map to:all,topic:%s|r:%i;c:%i",exchange.request.topic.c_str(),row,collum);
                    else
                        ROS_INFO("[MapMerger] Sended Map to:%s,topic:%s|r:%i;c:%i",exchange.request.dst_robot.c_str(),exchange.request.topic.c_str(),row,collum);
                    else
                        ROS_WARN("Destination host unreachable [%s](if nothing -> boradcast)",hostname.c_str());
                }
                else
                    ROS_INFO("[MapMerger] Could not call service to send map");
            }
        }
}
...
{
    ROS_INFO("[MapMerger] sendMetaData");
    //ros::nodeHandle nodeHandle ("~");
    ros::ServiceClient client;
...
{
    ROS_INFO("[MapMerger] findTransformIndex");
...
{
    ROS_INFO("[MapMerger] findRobotIndex");
...
{
    ROS_INFO("[MapMerger] createLogPath");
    full_log_path = log_path + std::string("/map_merger/") + robot_name;
    ROS_INFO("[MapMerger] Creating log path \"%s\".", full_log_path.c_str());
...
{
    ROS_INFO("[MapMerger] --> Top of service log_output_srv");
    if(!createLogPath())
        return false;
...
{
    ROS_INFO("[MapMerger] transformPointSRV");
    //need transform
...
{
    ROS_INFO("[MapMerger] sendBackAskedMapData");
    int min_x = map_width;
    int max_x = 0;
    int min_y = map_height;
    int max_y = 0;
    ...
    ROS_INFO("[MapMerger] Sending missed map updates to %s",robotName.c_str());
    std::vector<int>* containedUpdates = new std::vector<int>();
...
{
    ROS_INFO("[MapMerger] sendControlMessage");
...
{
    ROS_INFO("[MapMerger] Checking if robots are already in the network, service:%s/adhoc_communication/get_neighbors",robot_prefix.c_str());
    ...
    if(getNeighborsClient.call(getNeighbors))
    {
        ...
        if(getNeighbors.response.neigbors.size() > 0)
        {
            for(int i = 0; i < getNeighbors.response.neigbors.size();i++)
            {
                ...
                updateMan->addNewUpdateList();
                makeEmptyMapData(newRobotName,map_height,map_width,local_map->info.resolution);
                ROS_INFO("[MapMerger] asking %s for map",newRobotName.c_str());
                std::vector<int>* empty = new std::vector<int>();
